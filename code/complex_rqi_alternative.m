# DOES NOT WORK
function [x, sigma, iterations, eigval_iterates, eigvec_iterates, residuals] = complex_rqi_alternative(a,x,sigma,tolerance)
...
%   sigma and tol are also described there.
  ...
  while res >= tolerance
    res
    atilde = a + res*1i*speye(m);
    x = (atilde - sigma*speye(m))\x;
    x = real(x) / norm(real(x));
    sigma = (x' * atilde * x);
    res = norm((atilde - sigma*speye(m))*(x));
