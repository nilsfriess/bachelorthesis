...
following defintion provides us with a notion of the speed at which
...
the smallest eigenvalue $\lambda_1$. Combining this idea with
...
\[
  \vec{x}^{(k)} = {\left( \mat{A} - \sigma \mat{I} \right)}^{-1}
  \vec{x}^{(k-1)}\,,
\]
...
\todo{Check this again} to compute the inverse explicitly. Instead,
before the loop we can compute the $\mat{LU}$ decomposition of
$\mat{A} - \sigma \mat{I}$ and solve the system
\[
  {\left( \mat{A} - \sigma \mat{I} \right)} \vec{x}^{(k)} =
  \vec{x}^{(k-1)}
\]
for $\vec{x}^{(k)}$ reducing the complexity from $\O(n^3)$ to
$\O(n^2)$ at each step. We summarise the results in
...
already available. Note, however, that while a shift that is very
close to an eigenvalue increases the convergence rate, at the same
leads to a more and more ill-conditioned linear system. In the case
when $\sigma$ is exactly an eigenvalue the system is even singular and
one might expect the method to fail in these cases. It took some time
from when inverse iteration was first defined until Wilkinson
established that this is merely a problem in exact arithmetic and it
was fully understood what was observed in practice for many years,
namely that this poses no problem when the linear system is solved
numerically using a backwards stably method,
see~\cite[620\psq]{wilkinson} and~\cite{petersWilkinson}.
