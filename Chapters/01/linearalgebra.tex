...
\begin{remark}[Generalisations of eigenvalue problems]
  ...
  solve~\eqref{eq:eigvalproblem:general} often under certain
  assumptions posed on $\mat{M}$ such as positive
  ...
  \emph{eigenfunction}. Other generalisations include the
...
matrices. Since we are always interested in how fast these methods
produce a good approximation of eigenvector or eigenvalue (or both),
we first need the following defintion.

\begin{definition}[Order of Convergence]
  Let ${(\vec{x}_k)}_{k \in \N}$ be a sequence in $\R^n$ that
  ...
  \item The sequence is said to converge \emph{linearly} to $\vec{z}$,
    ...
    \begin{equation*}
      \lim_{k \rightarrow \infty} \frac{
        \norm{\vec{x}_{k+1} - \vec{z}}
      }{
        \norm{\vec{x}_{k} - \vec{z}}
      } < \rho\,,
    \end{equation*}
    where $M$ is called the \emph{rate of convergence}.
...
% Definition 6.3 (Konvergenzordnung). Sei (x k ) k≥0 ⊂ R n eine
% konvergente Folge mit Grenzwert x ∗ ∈ R n . Wir sagen (i) x k
% konvergiert gegen x ∗ mit Konvergenzordnung p > 1, falls C ≥ 0 und N
% ∈ N existieren, sodass für alle k ≥ N gilt kx k+1 − x ∗ k ≤ Ckx k −
% x ∗ k p .  (ii) x k konvergiert linear gegen x ∗ mit
% Konvergenzfaktor C ∈ [0, 1), falls N ∈ N existiert, sodass für alle
% k ≥ N gilt kx k+1 − x ∗ k ≤ Ckx k − x ∗ k .  1166 Iterative
% Verfahren für (nicht) lineare Gleichungssysteme Für p = 2 spricht
% man von quadratischer Konvergenz.
...
that the method converges linearly with convergence rate
\begin{equation}
...
approximation of an eigenvalue and wants to compute an eigenvector it
belongs to. The following method overcomes both of these drawbacks (at
least partly).
...
studied in the next chapter. For further discussion on the variants
and developments of these so called \emph{shift and invert} techniques
see, \eg~\cite{ipsenhistory, tapia2018, golub2000eigenvalue}.
