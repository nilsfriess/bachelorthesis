...
\begin{algorithm}[htbp]
  ...
  \For{$k = 1,2, \dotsc$ until convergence}{
    Solve ${\left( \mat{A} - \mu^{(k)} \mat{I}\right)} \vec{\tilde{x}}^{(k)} = \vec{x}^{(k-1)}$ for $\vec{\tilde{x}}^{(k)}$\;
...
rigorous justfication is given in~\cite[59\psqq]{saad2011}. Here, we
give only some important results without proof for the case when
$\mat{A}$ is symmetric. A popular result, usually referred to as the
\emph{Bauer-Fike theorem}~\cite{saad2011} \todo{page} states that
...
\begin{equation*}
  \abs{\lambda - \mu^{(k)}} < \mathtt{tol}\,.
\end{equation*}
For the eigenvector one can show that the following bound holds
\begin{equation*}
  \sin \angle(\vec{u}^{(k)}, \vec{v}) \le \frac{
    \norm{r}^{(k)}
  }{
    \delta
  }
