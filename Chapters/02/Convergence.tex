...
results concerning the global convergence behaviour.
...
practice the preasymptotic phase rarely takes more than two steps.
...
$\mat{Q}$. We see that running RQI with $\mat{A}$\todo{Derive this}
and $\vec{x}^{(0)}$ is equivalent to running RQI with $\mat{\Lambda}$
and $\hat{\vec{x}}^{(0)}$. Thus, we assume that
...
\begin{proof}[Proof of Theorem~\ref{thm:rqi:cubic}]
  ...
  (cf. Definition~\ref{def:eig:simple}) and suppose without loss of
  ...
  (cf. Lemma~\ref{lem:rq:quadestimate}). We obtain
  \begin{align*}
...
\end{equation*}
In some texts that study inverse iteration in a more particularized
...
current iterate vector choosen as the shift), see for
