...
two linear systems at each step instead of one.\todo{Check content
  of papers 4-6 of Ostrowski}
...
computed eigenvalue lies in a given interval. He obtains criteria to
switch back and forth between the two algorithms to benefit from their
respective advantages (inverse iteration is guarantueed to converge to
the nearest eigenvalue but convergence is merely linear; RQI posseses
local cubic convergence but the global convergence behaviour is
possibly erratic).
...
Besides the unpredictablity of the outcome of RQI another major
...
factorize the matrix beforehand as was the case in inverse
iteration. There are several obvious possiblities to reduce the cost
...
mentioned above can also be interpreted as a method to reduce the
