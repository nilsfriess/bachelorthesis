...
\begin{equation}
  ...
  \text{Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I})\, \vec{x}^{(k+1)} = \vec{e}_1\,,
\end{equation}
...
\begin{equation*}
  \text{Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{M})\, \vec{x}^{(k+1)} = \vec{e}_1\,.
\end{equation*}
...
following iteration
\begin{equation*}
  \text{Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I}) \vec{x}^{(k+1)} = \vec{e}_m\,,
\end{equation*}
...
\begin{equation}
  ...
  \text{Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I}) \vec{x}^{(k+1)} = \vec{x}^{(k)}\,.
\end{equation}
...
called the Dominant Pole Algorithm (DPA). Further, he observes that
RQI does not take much advantage of the information in the initial
vector $\vec{x}^{(0)}$. In other words, even if the initial vector is
a very good approximation of the wanted eigenvector, RQI might
...
system can be used to improve the outer convergence of inexact RQI. We
will briefly come back to the idea of solving the inner system
