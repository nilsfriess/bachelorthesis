...
In this section we give an overview on the history of RQI. We also
discuss recent contributions that are relevant to this thesis.  Some
of the results, mainly the ones that are concerned with the
...
\begin{equation}
  ...
  \text{``Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I})\, \vec{x}^{(k+1)} = \vec{e}_1\text{''}\,,
\end{equation}
where $\vec{e}_1$ denotes the first natural coordinate vector, \ie the
first column of the identity matrix, and $\vec{x}^{(k)}$,
$\vec{x}^{(k+1)}$ denote the current and next iterate,
respectively. In fact, Lord Rayleigh considered the generalised
eigenvalue problem and so in his text, the iteration reads
\begin{equation*}
  \text{``Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{M})\, \vec{x}^{(k+1)} = \vec{e}_1\text{''}\,.
\end{equation*}
...
following iteration
\begin{equation*}
  \text{``Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I}) \vec{x}^{(k+1)} = \vec{e}_m\text{''}\,,
\end{equation*}
where $\vec{e}_m$ is \emph{any} of the natural coordinate
vectors. Without a rigorous proof Kohn argues that
$\rq_{\mat{A}}(\vec{x}^{(k)})$ converges quadratically to an
...
\begin{equation}
  ...
  \text{``Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I}) \vec{x}^{(k+1)} = \vec{x}^{(k)}\text{''}\,.
\end{equation}
...
normalisation step. Crandall establishes cubic convergence of the
sequence of vector iterates and the approximate eigenvalue sequence
$\rq_{\mat{A}}(\vec{x}^{(k)})$. His analysis is based on the
assumption that the sequence of vectors $\vec{x}^{(k)}$ does converge.
To see why the assumption is wrong, we assume the contrary, \ie
suppose that the sequence converges to an eigenvector, say
$\vec{v}_j$.  From~\eqref{eq:unnormalised:rqi} we have
\begin{equation*}
  \quad \mat{A}\vec{v}_j - \rq_{\mat{A}}(\vec{v}_j) \vec{v}_j = \vec{v}_j
  \qquad \Leftrightarrow \qquad
  \mat{A}\vec{v}_j = (1 + \rq_{\mat{A}}(\vec{v}_j)) \vec{v}_j\,,
\end{equation*}
and so $\vec{v}_j$ is an eigenvector of $\mat{A}$ with corresponding
eigenvalue $1 + \rq_{\mat{A}}(\vec{v}_j)$. Since we know that for any
eigenvector, the value of $\rq_{\mat{A}}(\vec{v}_j)$ is the eigenvalue
it belongs to, this implies
\begin{equation*}
  \rq_{\mat{A}}(\vec{v}_j) = 1 + \rq_{\mat{A}}(\vec{v}_j)\,,
\end{equation*}
which is a contradiction. Also, Crandall establishes what is usually
referred to as $r$-cubic convergence. This is a weaker notion of
convergence than the one defined earlier, which is called $q$-cubic
convergence (see~\cite[Appendix~A.1]{tapia2018} for a detailed
definition of these notions of convergence).
...
\begin{equation}
  ...
  \text{``Solve}\quad (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I}) \vec{x}^{(k+1)} = \bm{\eta}\,,\quad \bm{\eta} \neq \vec{0}\text{''}\,.
\end{equation}
...
of~\eqref{eq:rqi:ostrowski:first} by the solution from the previous
step
\begin{equation}
  \label{eq:rqi:unnormalised:ostrowski}
  (\mat{A} - \rq_{\mat{A}}(\vec{x}^{(k)}) \mat{I}) \vec{x}^{(k+1)} = \vec{x}^{(k)}\,,
\end{equation}
...
$\mu_k \coloneqq \rq_{\mat{A}}(\vec{x}^{(k)})$, \ie
\begin{equation}
  \label{eq:rqi:cubic}
  \frac{
    \mu_{k+1} - \lambda
  ...
  \longrightarrow M \quad \text{ as } k \rightarrow \infty\,,
\end{equation}
where $\lambda$ is an eigenvalue of $\mat{A}$ and $M$ is a positive
constant. Local convergence means that $\vec{x}^{(0)}$ is assumed to
be near the eigenvector corresponding to $\lambda$.
...
discusses this in more detail and remarks that Crandall proved the
...
1974 Parlett~\cite{parlettrqi} generalised the result to the case when
$\mat{A}$ is normal. A slightly more concise version of the proof has
been again published in Parlett's book~\cite{Parlett1998}.
...
\begin{example}
  ...
  \item First consider RQI started with the vector
    \begin{equation*}
      ...
      \end{pmatrix}\,.
    \end{equation*}
    This results in the eigenpair $(1, \vec{e}_1)$ although the
    Rayleigh Quotient of the initial shift is
    $\rq_{\mat{A}}(\vec{x}^{(0)}) = 2.000770218344729$.  Note,
    however, that computing the angle between
    $\vec{x}^{(0)}$ and the eigenvectors gives
    \begin{equation*}
      \angle(\vec{x}^{(0)}, \vec{e}_1) \approx 0.61\,, \quad
      \angle(\vec{x}^{(0)}, \vec{e}_2) \approx 1.57\,, \quad
      \angle(\vec{x}^{(0)}, \vec{e}_3) \approx 0.95\,,
    \end{equation*}
    that is, the angle between the initial vector and the resulting
    eigenvector is smallest.
  \item Now take $\vec{x}^{(0)} = {\left( 0.74278,\ 0.55709,\
        0.37139\right)}^\tp$.  The corresponding angles are
    \begin{equation*}
      \angle(\vec{x}^{(0)}, \vec{e}_1) \approx 0.73\,, \quad
      \angle(\vec{x}^{(0)}, \vec{e}_2) \approx 0.97\,, \quad
      \angle(\vec{x}^{(0)}, \vec{e}_3) \approx 1.19\,,
    \end{equation*}
    and the initial shift is $\rq_{\mat{A}}(\vec{x}^{(0)}) \approx
    1.7241$. In this case, RQI converges to the eigenpair $(2,
    \vec{e}_2)$, \ie not the eigenvector that makes the smallest angle
    with
    $\vec{x}^{(0)}$ but rather the one that corresponds to the
    eigenvalue to which the initial shift is closest.
  \end{enumerate}
\end{example}
This example shows that there seems to be no obvious way how the
computed eigenpair depends on
$\vec{x}^{(0)}$ or
...
known beforehand, and so they are of little practical use.
...
and Fox~\cite{beattiefox} derive computable criteria that ensure that
the iterates and thus the computed eigenvalue stay within a given
interval; only the number of eigenvalues contained in the interval
needs to be known.
...
Recently, Rommes~\cite{rommes} derived sharper bounds for local
convergence neighbourhoods. He compares RQI with a related algorithm,
called the Dominant Pole Algorithm. Further, he observes that RQI does
not take much advantage of the information in the initial vector
$\vec{x}^{(0)}$. In other words, even if the initial vector is a very
good approximation of the wanted eigenvector, RQI might fail. We will
come back to this observation in the next chapter.
...
the Rayleigh Quotient and Szyld's paper~\cite{szyld} that we mentioned
above can also be interpreted as a method that reduces the
...
system can be used to improve the outer convergence of inexact RQI.
% We will briefly come back to the idea of solving the inner system
% inexactly when discussing our new method in the next chapter.
