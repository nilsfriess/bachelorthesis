...
of the results, mainly those concerned with the convergence of RQI,
are discussed in more detail in Section~\ref{sec:convergence}.
...
$\mu^{(k)} \coloneqq \rq_{\mat{A}}(\vec{x}^{(k)})$, \ie
\begin{equation}
  \label{eq:rqi:cubic}
  \frac{
    \mu^{(k+1)} - \lambda
...
\begin{example}
  ...
  \item First consider RQI started with the vector
    ...
    however, that computing the angle between $\vec{x}^{(0)}$ and the
    eigenvectors gives
    \begin{equation*}
  ...
  \item Now take
    $\vec{x}^{(0)} = {\left( 0.74278,\ 0.55709,\ 0.37139\right)}^\tp$.
