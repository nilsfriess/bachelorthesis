...
Iteration we give an overview on the historic developments of RQI. We
...
Algorithm~\ref{alg:rqi}. In this section we give an overview on some
important milestones within this 60 years. Large parts of this
overview are based on~\cite{tapia2018}.\todo{Page}
...
eigenproblem~\eqref{eq:general:eig} but for our comparitive purposes
it is sufficient to consider the case $\mat{M} = \mat{I}$. Note, that
this algorithm is RQI without the normalisation step. Based on the
(wrong) assumption that the sequence of vectors $\vec{x}^{(k)}$
converges, Crandall establishes cubic convergence for this
sequence. To see why the assumption is wrong, we assume the contrary,
\ie suppose $\vec{x}^{(i)} \longrightarrow \vec{v}_k$ for some
$k = 1, \dotsc, n$.  From~\eqref{eq:unnormalised:rqi} we have
\begin{equation*}
...
fixed vector $\vec{eta}$ in the right hand side
...
We now give an overview on the developments and contributions after
the introduction of RQI in 1958. We also look at some recent
contributions and try to identify their major focus.

After Ostrowski's proof of the local cubic convergence, it took
another ten years until the question on the global convergence
behaviour of RQI was answered. In 1968 Parlett and
Kahan~\cite{parlettkahan} showed that RQI applied to symmetric
matrices converges for almost all starting vectors and in 1974
Parlett~\cite{parlettrqi} generalised the result for the case when
$\mat{A}$ is normal. A slightly more concise version of the proof has
been again published in Parlett's book~\cite{Parlett1998}. However,
since parts of the proof are very technical and the proof does not
give much insight in the method (at least for our purposes), we will
omit it.

Of course, not only is it of interest \emph{that} the method converges
but also \emph{which} eigenpair it converges to.  The following
example demonstrates that the convergence behaviour of RQI might
...
\begin{example}
  ...
  \item First consider RQI started with the vector
    \begin{equation*}
      ...
      \end{pmatrix}\,.
    \end{equation*}
    This produces the eigenpair $(1, \vec{e}_1)$ altough
    \begin{equation*}
  ...
  \item Now take
    ...
    eigenvector that has the smallest angle with $\vec{x}^{(0)}$.
  \end{enumerate}
\end{example}
This example shows that there seems to be no obvious way how the
computed eigenpair depends on $\vec{x}^{(0)}$ or
$\rq_{\mat{A}}(\vec{x}^{(0)})$, and indeed the characterization of the
...
\emph{convergence neighborhoods} for the shift, \ie intervals around
...
Efforts have beend made to obtain local convergence regions with as
...
contained in the interval is known. Szyld~\cite{szyld} suggests
combining shifted inverse iteration and RQI to ensure that the
computed eigenvalue lies in a given interval. In particular, he
suggested running a few iterations of shifted inverse iteration (since
it is guarantueed to converge to the nearest eigenvalue) and then
switching to RQI (to benefit from the cubic convergence) and studied
criteria for deciding when to switch from one method to the other.
...
``There appears to be no simple description of how $\vec{v}$ depends
