...
means that
\begin{enumerate}
\item it is assumed that the sequence $(\vec{x}^{(k)})_i$ does
...
moment and change variables in RQI\footnote{Recall the notation from
  Algorithm~\ref{alg:rqi} where $\vec{y} = \vec{y}^{(k)}$ denotes the
  unnormalised $k$th iterate and $\vec{x} = \vec{x}^{(k)}$ denotes the
  same iterate after normalisation.}  to
$\hat{\vec{x}} \coloneqq \mat{Q}^\tp \vec{x}$ and
$\hat{\vec{y}} \coloneqq \mat{Q}^\tp \vec{y}$. Then
\begin{equation*}
  \mu = \rq_{\mat{A}}(\vec{x}) = \frac{
    \vec{x}^\tp \mat{A} \vec{x}
  }{
    \vec{x}^\tp \vec{x}
  } = \frac{
...
\end{equation*}
and
$\mat{Q} \hat{\vec{y}}^{(k+1)} = {(\mat{A} -
  \mu^{(k)}\mat{I})}^{-1}\mat{Q}\hat{\vec{x}}^{(k)}$. Hence,
\begin{align*}
  \hat{\vec{y}}^{(k+1)} &= \mat{Q}^\tp {(\mat{A} - \mu^{(k)}\mat{I})}^{-1}\mat{Q}\hat{\vec{x}}^{(k)} \\
                        &= {(\mat{Q}^\tp \mat{A} \mat{Q} - \mu^{(k)}\mat{I})}^{-1}\hat{\vec{x}}^{(k)}\\
                        &= {(\mat{\Lambda} - \mu^{(k)}\mat{I})}^{-1}\hat{\vec{x}}^{(k)}\,,
\end{align*}
where we used $\mat{Q}^\tp = \mat{Q}^{-1}$, the orthogonality of
$\mat{Q}$. We see that running RQI with $\mat{A}$ and $\vec{x}^{(0)}$
is equivalent to running RQI with $\mat{\Lambda}$ and
$\hat{\vec{x}}^{(0)}$. Thus, we assume that $\mat{A} = \mat{\Lambda}$
is already diagonal which in particular implies that the eigenvectors
of $\mat{A}$ are $\vec{e}_i$, the natural coordinate vectors.
\begin{proof}[Proof of Theorem~\ref{thm:rqi:cubic}]
  Suppose without loss of generality that $\vec{x}^{(k)}$ converges to
  $\vec{e}_1$. Remember that we assume that the current iterate is a
  sufficiently good estimate of $\vec{e}_1$, such that for some $k$ we
  can write $\vec{x}^{(k)} = \vec{e}_1 + \vec{d}^{(k)}$ with
  $\norm*{\vec{d}^{(k)}} = \epsilon \ll 1$. To show cubic convergence,
  we have to verify that
  \begin{equation*}
    \lim_{i \rightarrow \infty} \frac{
      \norm*{\vec{x}^{(k+1)} - \vec{e}_1}
    }{
      \norm*{\vec{x}^{(k)} - \vec{e}_1}^3
    }
    <
    M\,,
  \end{equation*}
  for some positive constant $M$. We know that
  $\norm*{\vec{x}^{(k)} - \vec{e}_1}^3 = \epsilon^3$, hence it
  suffices to show that
  $\norm*{\vec{x}^{(k+1)} - \vec{e}_1} = \mathcal{O}(\epsilon^3)$. In
  other words we have to show that
  $\vec{x}^{(k+1)} = \vec{e}_1 + \vec{d}^{(k+1)}$ with
  $\norm*{\vec{d}^{(k+1)}} = \mathcal{O}(\epsilon^3)$.

  Since the vectors are normalised at each step we have
  \begin{align*}
    1 = {(\vec{x}^{(k)})}^\tp (\vec{x}^{(k)})
    &= {(\vec{e}_1 + \vec{d}^{(k)})}^\tp (\vec{e}_1 + \vec{d}^{(k)}) \\
    &= \vec{e}_1^\tp \vec{e}_1 + 2 \vec{e}_1^\tp \vec{d}^{(k)} + {(\vec{d}^{(k)})}^\tp \vec{d}^{(k)} \\
    &= 1 + 2 d^{(k)}_1 + \epsilon^2\,,
  \end{align*}
  where $d^{(k)}_1$ denotes the first component of the vector
  $\vec{d}^{(k)}$. Rewriting gives $d^{(k)}_1 = -\epsilon^2/ 2$ and
  ...
  \begin{align*}
    \mu^{(k)} &= {(\vec{x}^{(k)})}^\tp \mat{\Lambda}\vec{x}^{(k)} \\
              &= {(\vec{e}_1 + \vec{d}^{(k)})}^\tp \mat{\Lambda} {(\vec{e}_1 + \vec{d}^{(k)})} \\
              &= \vec{e}_1^\tp \mat{\Lambda} \vec{e}_1 + 2\vec{e}_1^\tp \mat{\Lambda} \vec{d}^{(k)} + {(\vec{d}^{(k)})}^\tp \mat{\Lambda} \vec{d}^{(k)}\\
              &= \lambda_1 - \eta\,,
  \end{align*}
  where
  \begin{equation*}
    \eta \coloneqq -2 \vec{e}_1^\tp \mat{\Lambda} \vec{d}^{(k)} -
    {(\vec{d}^{(k)})}^\tp \mat{\Lambda} \vec{d}^{(k)} = \lambda_1
    \epsilon^2 - {(\vec{d}^{(k)})}^\tp \mat{\Lambda} \vec{d}^{(k)}\,.
  \end{equation*}
  ...
  \end{equation}
  and we see that
  $\mu^{(k)} = \lambda_1 - \eta = \lambda_1 + \mathcal{O}(\epsilon^2)$
  (note that this proves Lemma~\ref{lem:rq:quadestimate}, the
  quadratic accuracy of the Rayleigh Quotient). We obtain \begingroup
  \allowdisplaybreaks
  \begin{align*}
    \vec{y}^{(k+1)}&= {(\mat{\Lambda} - \mu^{(k)})}^{-1} \vec{x}^{(k)} \\
                   &= {\left(
                     \frac{x^{(k)}_1}{\lambda_1 - \mu^{(k)}},
                     \frac{x^{(k)}_2}{\lambda_2 - \mu^{(k)}},
                     \cdots,
                     \frac{x^{(k)}_n}{\lambda_n - \mu^{(k)}}
                     \right)}^\tp\\
                   &= {\left(
                     \frac{1 + d^{(k)}_1}{\lambda_1 - \mu^{(k)}},
                     \frac{d^{(k)}_2}{\lambda_2 - \mu^{(k)}},
                     \cdots,
                     \frac{d^{(k)}_n}{\lambda_n - \mu^{(k)}},
                     \right)}^\tp\\
                   &= {\left(
                     \frac{1 - \frac{\epsilon^2}{2} }{\eta},
                     \frac{d^{(k)}_2}{\lambda_2 - \lambda_1 + \eta},
                     \cdots,
                     \frac{d^{(k)}_n}{\lambda_n - \lambda_1 + \eta},
                     \right)}^\tp \\
                   &= \frac{1 - \frac{\epsilon^2}{2}}{\eta} \,
                     {\left(
                     1,
                     \frac{d^{(k)}_2 \eta}{(1 - \frac{\epsilon^2}{2})(\lambda_2 - \lambda_1 + \eta)},
                     \cdots,
                     \frac{d^{(k)}_n \eta}{(1 - \frac{\epsilon^2}{2})(\lambda_n - \lambda_1 + \eta)},
                     \right)}^\tp \span \span \\
                   &\eqqcolon \frac{1 - \frac{\epsilon^2}{2}}{\eta}\, (\vec{e}_1 + \hat{\vec{d}}^{(k+1)})\,,
  \end{align*}
  \endgroup where in the first step we used that the inverse of a
  diagonal matrix consists of the inverse of the entries on the
  diagonal. The next step holds since
  $\vec{x}^{(k)} = \vec{e}_1 + \vec{d}^{(k)}$ by assumption. After
  that we use that we can write $\mu^{(k)} = \lambda_1 - \eta$ and
  $d^{(k)}_1 = -\epsilon^2/2$.  If we denote again by $\delta$ the gap
  between $\lambda_1$ and the rest of the spectrum, \ie
  \begin{equation*}
    \delta \coloneqq \min_{j=2}^n \abs{\lambda_j - \lambda_1}\,,
  \end{equation*}
  we can bound the denominators of $\hat{\vec{d}}^{(k+1)}$ using
  ...
  \begin{equation*}
    ...
    \frac{
      \norm*{\vec{d}^{(k)}} \abs*{\eta}}{
      (1 - \frac{\epsilon^2}{2})(\delta - \abs*{\eta})}
    \le
    \frac{
      2\norm{\Lambda}\epsilon^3}{
      (1 - \frac{\epsilon^2}{2})(\delta - 2 \norm{\Lambda}\epsilon^2)
    }
  \end{equation*}
  or $\norm*{\hat{\vec{d}}^{(k+1)}} =
  \mathcal{O}(\epsilon^3)$. Finally, since
  $\vec{x}^{(k+1)} = \vec{e}_1 + \vec{d}^{(k+1)} = \vec{y}^{(k+1)} /
  \norm*{\vec{y}^{(k+1)}}$ and
  \begin{equation*}
    \frac{
      \vec{y}^{(k+1)}}{
    ...
    \frac{
      \frac{1 - \epsilon^2 / 2}{\eta}\, (\vec{e}_1 + \hat{\vec{d}}^{(k+1)})
    }{
      \norm{\frac{1 - \epsilon^2 / 2}{\eta}\, (\vec{e}_1 + \hat{\vec{d}}^{(k+1)})}
    } =
    \frac{
      \vec{e}_1 + \hat{\vec{d}}^{(k+1)}
    }{
      \norm{\vec{e}_1 + \hat{\vec{d}}^{(k+1)}}
    }
  \end{equation*}
  we see that also $\norm{\vec{d}^{(k+1)}} = \mathcal{O}(\epsilon^3)$,
...
\end{equation*}
In some texts that study inverse iteration in greater detail, cubic
convergence of RQI is proved by combining the quadratic accuracy of
the Rayleigh Quotient together with convergence results from inverse
iteration (since a single step of RQI is equivalent to a single step
of inverse iteration with the Rayleigh Quotient of the current iterate
vector chosen as the shift), see for example~\cite[208]{trefethen1997}
or~\cite[77--79]{boerm}.

To conclude this section we briefly discuss some of the results
...
\begin{proof}[Proof of Lemma~\ref{lem:monotoni:res}]
  \begin{align*}
    ...
    &\le \norm{\left( \mat{A} - \mu^{(k)} \mat{I} \right) \vec{x}^{(k+1)}} && \text{($\mu^{(k+1)}$ minimises the residual norm)} \\
    ...
    &= \norm{{\left( \vec{x}^{(k)} \right)}^\herm \left(\mat{A} - \mu^{(k)} \mat{I}\right)} && \text{($\vec{x}^{(k+1)}$ is a unit vector)} \\
    &= \norm{\left(\mat{A} - \mu^{(k)} \mat{I}\right)\vec{x}^{(k)}} && \text{($\mat{A}$ is symmetric)} \\
    &= \norm{\vec{r}^{(k)}} \,.
  \end{align*}
...
\begin{theorem}[Global convergence of RQI]
  ...
  \item the sequence $\mu^{(k)}$ converges to
    ...
    $\vec{v}_{pq}^-$, where
    $\vec{v}_{pq}^\pm = (\vec{v}_p \pm \vec{v}_q) / \sqrt{2}$ and
    $\lambda_p$, $\lambda_q$ are eigenvalues of $\mat{A}$ with
...
proof by Parlett in~\cite{Parlett1998}. We begin by observing that the
monotonicity of the residuals implies
\begin{gather*}
...
the Rayleigh Quotient is a continuous function on the unit sphere
Parlett concludes that
\begin{gather*}
...
require a number of auxiliary results relating the eigenvectors of
$\mat{A}$ to its square $\mat{A}^2$. Therefore, as mentioned above, we
do not discuss this part of the proof here.

\begin{example}
  To illustrate the situation in the second part of the theorem
  consider RQI applied to the diagonal matrix
  $\mat{A} = \diag(1,\ 2,\ 4)$ with initial vector
  \begin{gather*}
    \vec{x}^{(0)} = {\left( \frac{1}{\sqrt{2}}\quad
        \frac{1}{\sqrt{2}}\quad 0 \right)}^\tp\,.
  \end{gather*}
  It is easy to check that this results in $\mu^{(k)} = 1.5$ and
  \begin{gather*}
    \vec{x}^{(2k)} = {\left( \frac{1}{\sqrt{2}}\quad
        \frac{1}{\sqrt{2}}\quad 0 \right)}^\tp\qquad \text{and}\qquad
    \vec{x}^{(2k+1)} = {\left( -\frac{1}{\sqrt{2}}\quad
        \frac{1}{\sqrt{2}}\quad 0 \right)}^\tp
  \end{gather*}
  for $k = 0,1,2\dotsc$. The instability mentioned in the theorem
  essentially prevents the method from failing in practice since
  roundoff errors will inevitably introduce perturbations of the
  vector iterate. However, this can drastically increase the number of
  steps before the cubic convergence takes place. In this particular
  example, when executed in Matlab~\cite{MATLAB} the method requires
  36 steps in total.
\end{example}
...
proved~\cite[625]{battersonsmillie}. They proceed to give a proof of
