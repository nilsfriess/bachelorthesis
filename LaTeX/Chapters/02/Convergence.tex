...
\begin{proof}[Proof of Theorem~\ref{thm:rqi:cubic}]
  ...
  \begin{align*}
    \mu^{(i)} &= {(\vec{x}^{(i)})}^\tp \mat{\Lambda}\vec{x}^{(i)} \\
              ...
              &= \vec{e}_1^\tp \mat{\Lambda} \vec{e}_1 + 2\vec{e}_1^\tp \mat{\Lambda} \vec{d}^{(i)} + {(\vec{d}^{(i)})}^\tp \mat{\Lambda} \vec{d}^{(i)}\\
              &= \lambda_1 - \eta\,,
  \end{align*}
  where
  \begin{equation*}
  ...
  \begin{align*}
    \vec{y}^{(i+1)}&= {(\mat{\Lambda} - \mu^{(i)})}^{-1} \vec{x}^{(i)} \\
                   ...
                   &= \frac{1 - \frac{\epsilon^2}{2}}{\eta} \,
                     {\left(
                     1,
                     \frac{d^{(i)}_2 \eta}{(1 - \frac{\epsilon^2}{2})(\lambda_2 - \lambda_1 + \eta)},
                     \cdots,
                     \frac{d^{(i)}_n \eta}{(1 - \frac{\epsilon^2}{2})(\lambda_n - \lambda_1 + \eta)},
                     \right)}^\tp \span \span \\
                   &\eqqcolon \frac{1 - \frac{\epsilon^2}{2}}{\eta}\, (\vec{e}_1 + \hat{\vec{d}}^{(i+1)})\,,
  \end{align*}
  ...
  next step holds since $\vec{x}^{(i)} = \vec{e}_1 + \vec{d}^{(i)}$ by
  assumption. After that we use that we can write
  $\mu^{(i)} = \lambda_1 - \eta$ and $d^{(i)}_1 = -\epsilon^2/2$.  If
...
\begin{theorem}[Global convergence of RQI]
  ...
  \item the sequence $\mu^{(k)}$ converges to
    ...
    $\vec{v}_{pq}^-$, where
    $\vec{v}_{pq}^\pm = (\vec{v}_p \pm \vec{v}_q) / \sqrt{2}$, where
