...
\begin{proof}[Proof of Theorem~\ref{thm:rqi:cubic}]
  ...
  quadratic accuracy of the Rayleigh Quotient). We obtain
  \begin{align*}
  ...
  \end{align*}
  where in the first step we used that the inverse of a diagonal
  matrix consists of the inverse of the entries on the diagonal. The
  next step holds since $\vec{x}^{(k)} = \vec{e}_1 + \vec{d}^{(k)}$ by
  assumption. After that we use that we can write
  $\mu^{(k)} = \lambda_1 - \eta$ and $d^{(k)}_1 = -\epsilon^2/2$.  If
  we denote again by $\delta$ the gap between $\lambda_1$ and the rest
  of the spectrum, \ie
  \begin{equation*}
