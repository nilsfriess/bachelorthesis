...
showing that RQI is globally convergent is the follwoing fact which,
...
\end{equation*}
We also note that we can relate the $k$-th and $k+1$-th iterate of RQI
by
\begin{equation*}
...
\begin{lemma}[Monotonic residuals]
...
\begin{theorem}[Global convergence of RQI]
  Let the RQI be applied to a normal matrix started with any unit
  vector $\vec{x}^{(0)}$.  Then, as $k \rightarrow \infty$, the
  ...
  \item the sequence $\mu^{(k)}$ converges to
    ...
    $\lambda_p$ andd $\lambda_q$ are eigenvalues of $\mat{A}$ with
    corresponding eigenvectors $\vec{v}_p$ and $\vec{v}_q$.
  \end{enumerate}
  The latter is unstable under perturbations of $\vec{x}^{(k)}$.
\end{theorem}
As mentioned above, we do not give a proof of this result. Parlett
states in~\cite[61]{Parlett1998} and in~\cite[680]{parlettrqi} that
RQI converges for almost all starting vectors but Batterson and
