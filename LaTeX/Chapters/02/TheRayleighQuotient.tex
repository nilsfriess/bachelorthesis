...
We postpone the proof of this statement until when we discuss the
convergence behaviour of RQI.
...
Chapter~\ref{chapter:intro}. Now that we the defined the Rayleigh
...
computed. Therefore, for this \emph{residual-based error control} to
work, the computation of $\mu^{(k)}$ has to be added to these
algorithms.
...
results without proof for the case when $\mat{A}$ is Hermitian. A
popular result, usually referred to as the \emph{Bauer-Fike theorem}
for Hermitian matrices (see, \eg~\cite[59]{saad2011}) states that
