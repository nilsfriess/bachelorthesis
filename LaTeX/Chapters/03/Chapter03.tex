...
In this chapter we introduce \emph{Complex Rayleigh Quotient
  ...
  chapter by \emph{CRQI}.}).  This is a novel shift and invert type
algorithm similar to classic RQI that overcomes some of its
disadvantages. \todo{Finish intro for chapter}.
...
between the target eigenvalue $\lambda_k$ and the neighboring
...
$\mat{I} - \vec{u} \vec{u}^\tp$, interpreted as a linear mapping,
defines the orthogonal projection onto the span of $\vec{u}$. Of
course, this works for any vector $\vec{v} \in \R^n$ and can be seen
as follows.  Given a vector $\vec{x} \in \R^n$, the projection of
$\vec{x} \in \R^n$ onto the span of $\vec{v}$ is
\begin{equation*}
  \frac{
...
product on $\R^n$. Thus, the projection onto the orthogonal complement
of the span of $\vec{v}$ is
\begin{equation*}
...
will bareley ``see'' the imaginary part
...
\end{equation}
Now recall that $\vec{u}$ is assumed to be a good approximation of an
eigenvector $\vec{v}_k$ of $\mat{A}$. This also implies that the
orthogonal subspace of the span of $\vec{u}$ approximates the
orthogonal subspace spanned by $\vec{v}_k$. However, the latter is the
space spanned by the remaining eigenvectors of $\mat{A}$, since the
eigenvectors are pairwise orthogonal. Therefore, we expect that the
eigenvectors of $\tilde{\mat{A}}$ are similar to those of $\mat{A}$
and that the eigenvalues corresponding to eigenvectors $\vec{v}_j$,
$j \neq k$ to approximately $\lambda_j - \gamma i$ due
...
\begin{lemma}
  ...
  $\lambda_j(\tilde{\mat{A}}^{(0)}) = \lambda_j - \gamma i$ for
  $j \neq k$ and $\lambda_k(\tilde{\mat{A}}^{(0)}) = \lambda_k$.
\end{lemma}
\todo{Introduce eigenvalue notation in chapter 1}
\begin{proof}
  First consider $j \neq k$. We have
  \begin{equation*}
...
\end{equation*}
Weyl`s perturbation inequality then gives the bound
\begin{equation}
  \label{eq:weyl}
  \max_{k = 1}^n\, \abs*{\lambda_k(\mat{\tilde{A}}) - \lambda_k(\tilde{\mat{A^{0}}})} \le \norm{\tilde{\mat{A}}^{(1)}}\,.
\end{equation}
To compute the norm on the right hand side of~\eqref{eq:weyl} we make
use of the following well-known facts from linear algebra.
\begin{proposition}
