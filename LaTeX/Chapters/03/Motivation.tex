...
\todo{Rewrite} eigenvalues of $\tilde{\mat{A}}_{(0)}$ due to the
perturbation $\tilde{\mat{A}}_{(1)}$ to be small. The following lemma
answers how ``big'' this perturbation is.
...
summarise the idea of CRQI in Algorithm~\ref{alg:crqi:first}. Note tha
this is not the full algorithm but only the inner loop. Since we will
shortly change the algorithm slightly, we postpone the complete
