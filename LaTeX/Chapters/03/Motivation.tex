...
As $\gamma^{(k)} \rightarrow 0$ then linear system that is solved at
...
sparsity is preserved. We begin with the following auxiliary result.
...
solving the linear from Algorithm~\ref{alg:crqi:first}. Since the
result gets normalised immediately after, both solutions will result
in the same vector that is used in the next iteration. The Rayleigh
Quotient that is computed using this vector consequently is the same
in either of the variants.
...
additional steps are required compard to classic RQI. Moreover, we did
not encounter an example where CRQI failed to converge to any
eigenpair.

This concludes the motivation part of this section. As mentioned in
