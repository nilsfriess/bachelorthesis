...
\[
  \vec{u} = \sum_{i=1}^n \beta_i \vec{v}_i
\]
for certain $\beta_1, \dotsc, \beta_n \in \R$. Suppose now that
$\vec{u}$ is a good approximation for one of the eigenvectors, say for
$\vec{v}_k$. Then
\[
  \beta_k \approx 1 \qquad \text{and} \qquad \beta_j \approx 0 \
  \text{ for } \ j \neq k \,.
\]
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
As mentioned previously, in classic RQI even good approximations of
eigenvectors can lead to convergence to the wrong eigenpair. We will
see in Section~\ref{sec:crqi:experiments} that classic RQI seems to
barely make use of the information incorporated in the initial vector.
The main goal of complex RQI is to fix this, \ie to alter classic RQI
in such a way that for sufficiently good approximations of
eigenvectors convergence to this very eigenvector is guaranteed.
...
\vec{v}_k$. In the figure below we plotted the spectrum of $\mat{A}$
together with the spectrum of the perturbed matrix $\tilde{\mat{A}}$
for $\gamma = 1$. In this example we had
$\vec{u}^\tp \vec{v}_k \approx 0.83568$, where $\vec{v}_k$ is the
target eigenvector.  The wanted eigenvalue stays near the real line
while the rest of the spectrum gets ``pushed down'' into the complex
plane.\footnote{The matrix $\mat{A}$ we used here is a
  $200 \times 200$ Martin-Wilkinson matrix which is defined in the
  next section.}

\begin{figure}[htpb]
  \centering
  \includegraphics[width=0.85\textwidth]{Figures/perturbed_spectrum}
  \caption{Spectrum of the original problem and the perturbed
    problem}%
  \label{fig:perturbed_spectrum}
\end{figure}
...
eigenvalues of $\tilde{\mat{A}}_{(0)}$ due to the perturbation
$\tilde{\mat{A}}_{(1)}$ to be small. The following lemma answers how
``big'' this perturbation is.
...
We can now give an estimate of the eigenvalues of $\tilde{\mat{A}}$.
...
behaviour is discussed in Section~\ref{sec:crqi:experiments}. We
summarise the idea of CRQI in Algorithm~\ref{alg:crqi:first}. Note
that this is not the full algorithm but only the inner loop. Since we
will shortly change the algorithm slightly, we postpone the complete
definition until later.

\begin{algorithm}[htpb]
  \DontPrintSemicolon%
  \For{$k = 1,2,\dotsc$ until convergence}{
    Solve $\left( \mat{A} - i \gamma^{(k)}\left(\mat{I} - \vec{x}^{(k)} {(\vec{x}^{(k)})}^\ast \right) - \mu^{(k)}\mat{I} \right) \tilde{\vec{x}}^{(k+1)} = \vec{x}^{(k)}$\;
    $\vec{x}^{(k+1)} \gets \tilde{\vec{x}}^{(k+1)} / \norm*{\tilde{\vec{x}}^{(k+1)}}$\;
    $\mu^{(k+1)} \gets {(\vec{x}^{(k+1)})}^\herm \mat{A} \vec{x}^{(k+1)}$\;
    $\gamma^{(k+1)} \gets \norm*{(\mat{A} - \mu^{(k+1)}\mat{I}) \vec{x}^{(k+1)}}$\;
  }
  \caption{Complex Rayleigh Quotient Iteration (first version)}\label{alg:crqi:first}
\end{algorithm}

As $\gamma^{(k)} \rightarrow 0$ the linear system that is solved at
each step becomes the same system as in classic RQI.\@ The remaining
steps are essentially the same except that every transpose (\eg in the
computation of the Rayleigh Quotient) has to be replaced by a complex
conjugate transpose since the vectors might now contain a non-zero
imaginary part.
...
we will see in the next section, however, if good approximations of
the wanted eigenvector are used for the initial vector, the method
often requires merely three or four additional steps compared to
classic RQI.

Another problem seems to be that if $\mat{A}$ is sparse,
$\tilde{\mat{A}}$ will in general not be. In fact, the perturbed
matrix will quite possibly posses only few, if any, zero
...
$i \gamma \mat{I}$. The way we introduced the method above was merely
for motivational purposes. The resulting matrix is still complex
symmetric but now only the diagonal entries are altered and sparsity
is preserved. We begin with the following auxiliary result.
...
\begin{lemma}
  ...
  \begin{equation*}
    ...
    \mat{C} \coloneqq { \mat{A} - \mu \mat{I} - i \gamma( \mat{I} - \vec{u} \vec{u}^\herm) } \,,
  \end{equation*}
...
\begin{proof}
  ...
  Sherman-Morrison formula (see, for example, \cite[50]{golub1996})
  and letting
...
)$. Consider the linear system from Algorithm~\ref{alg:crqi:first}.
The lemma states that there exists a scalar $\alpha \in C$ such that
...
the factor $\alpha$ is a scalar multiple of the result obtained by
solving the linear system from Algorithm~\ref{alg:crqi:first}. Since
the result gets normalised immediately after, both solutions will
result in the same vector that is used in the next iteration. The
Rayleigh Quotient that is computed using this vector consequently is
the same in either of the variants.

Thus, it does indeed suffice to perturb the matrix only by the
diagonal matrix $-i \gamma^{(k)} \mat{I}$. We can collect the real
shift $\mu^{(k)}$ and the imaginary shift $i \gamma^{(k)}$ into a
complex-valued shift
\begin{equation*}
  \sigma^{(k)} \coloneqq \mu^{(k)} + i\gamma^{(k)}
\end{equation*}
and can now fully define Complex Rayleigh Quotient Iteration as it is
given in Algorithm~\ref{alg:crqi}.
...
might be ``left'' in the vector approximation
($\operatorname{Re}(\cdot)$ extracts the real part of a complex
vector). Since we again use a stopping criterion based on the norm of
the residual, the same error bounds we discussed earlier in
Chapter~\ref{chapter:rqi} hold and thus the vector iterates could
still contain a small imaginary part. This does also imply that the
final residual norm $\norm*{\mat{A}\vec{x} - \mu \vec{x}}$ might be
slightly larger than the residual norm computed in the last
iteration. In practice, however, we observed that the final residual
norm is still of the same order as the norm of the residual computed
...
Hence, we cannot expect that CRQI possesses the same appealing
convergence properties as classic RQI. However, numerical examples
suggest that convergence is still fast and often merely three or four
additional steps are required compared to classic RQI. Moreover, we did
not encounter an example where CRQI failed to converge to any
eigenpair.

This concludes the motivation part of this chapter. As mentioned in
the beginning of this thesis, our aim is not to provide a detailed
mathematical analysis of the method. Instead, we discuss various
numerical examples to demonstrate that the idea introduced above
works.
