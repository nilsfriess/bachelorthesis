...
We start with an example where the algorithm was run many times with a
fixed initial vector but varying initial shifts. To obtain the shifts
we computed the spectrum of the matrix using built-in functions of
Matlab and extracted 100 evenly spaced values form the interval
...
larger value than any of the others, Figure~\ref{fig:varying shift
  random}). In all of the remaining figures $N$ denotes the matrix
order and $\alpha$ denotes the angle between the initial vector and
...
close approximations of an eigenvector. In contrast, it appears that
...
negative or positive but rather its location in the spectrum (see also
the results using the other test matrices given in Figures). If it is
...
the initial shifts could be chosen accordingly. If one does not have
any a priori knowledge about the distribution of the eigenvalues it
seems best to use the algorithm as defined above, \ie to use the
Rayleigh Quotient as the initial shift. Even in cases where the
outcome of the method varied with changing initial shift, using the
Rayleigh Quotient often produced the correct result, see for instance
the first two examples of Figure~\ref{fig:real_shift:mat5}. Thus, for
the remainder of the thesis, CRQI is run with the initial real shift
set to the Rayleigh Quotient of the initial vector.
...
good of an approximation the initial vector needs to be. We do this by
studying the relationship between the angle that the initial vector
makes with the target eigenvector and the outcome of the method. To
that end, artificial approximations of the wanted eigenvector are
created as explained above where the contribution in the target
direction is successively increased throughout the experiment. We will
see that
...
initial vector and the wanted eigenvector and the computed eigenvalue
for classic RQI and CRQI. This particular example shows the results
for the Laplace matrices of different sizes; the results for the
remaining test problems are given in Figures~\ref{fig:angle:mat2}
to~\ref{fig:angle:mat4} in the Appendix.
...
$\pi / 4$; sometimes higher angles also produce the correct result but
taking into account all the expirments we found that it seems almost
impossible to make CRQI fail for angles below $\pi / 4$.
