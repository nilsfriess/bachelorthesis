...
\begin{example}
  ...
  decreases as the initial vector becomes closer to the target
  eigenvector. Classic RQI, on the other hand, sometimes requires a
  higher number of iterations even for small angles while still
  computing the wrong result. This again portrays another aspect of
  the erratic behaviour of classic RQI. For the test problems where
  the spacing between the eigenvalues is very small, classic RQI often
  produced very few correct results while CRQI showed the same
  behaviour as describe above -- independent of the matrix's size and
  hence the distance between the eigenvalues (see for example
  Figures~\ref{fig:ex4:mat2:a} or~\ref{fig:ex4:mat4:d}).
  ...
  and~\ref{fig:ex4:mat4:fail} in the Appendix.
\end{example}
