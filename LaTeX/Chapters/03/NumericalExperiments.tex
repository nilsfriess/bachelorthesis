...
that we mentioned several times before. This is not done seperately
but by running the same experiments for both methods and comparing the
results throughout. All experiments were executed in Matlab
9~\cite{MATLAB}. The criterion for convergence is
...
algorithms related to classic RQI, see, for example,~\cite{dax2003}
and the references therein. They are described below. Recall that we
are particularly interested in problems with closely spaced
eigenvalues and, except for the first two examples, all of the test
matrices will lead to such problems. Since in three of the test
problems the spacing even becomes closer the larger the matrix gets,
we will also study the algorithm's behaviour under increasing matrix
size.
...
others, e.\,g., $w_{50} = 10$ (in most examples the index of the component
...
complex RQI even finished within the same number of iterations. Still,
even in these cases, classic RQI often failed to converge to the
target eigenpair.
\end{example}
...
given in Figures~\ref{fig:real_shift:mat2}
...
initial vector was chosen completely random, \ie is \emph{not} an
...
\begin{figure*}[htpb]
  ...
  target for the Laplce matrix]{Plot of the computed eigenvalue
...
spectrum. More imporantly, the observations seem to be independent of
the size of the matrix and thus, independent of the spacing of the
eigenvalues.

\todo{Refer to next example for plots of this phenomenon}
\end{example}

\todo{Überleitung} What we did barely discuss in the previous examples
is the speed of CRQI compared to classic RQI. This is what most of the
remainder of the section is devoted to.
...
\begin{example}
  ...
  complex RQI, respectively. We also indicated whether or not the
  ...
  approximately below $45^\circ$. Classic RQI, on the other hand,
  sometimes requires a higher number of iterations even for small
  angles while still computing the wrong result. This again portrays
  another aspect of the erratic behaviour of classic RQI. For the test
  problems where the spacing between the eigenvalues is very small,
  classic RQI often produced very few correct results while CRQI
  showed the same behaviour as describe above -- independent of the
  matrix's size and hence the distance between the eigenvalues (see
  for example Figures~\ref{fig:ex4:mat2:a} or~\ref{fig:ex4:mat4:d}).
  ...
  angles becomes smaller. However, the
\end{example}
