...
complex RQI can be found in the Appendix. Note, that the methods are
...
to the Rayleigh Quotient of the initial vector and the initial
...
algorithms related to classic RQI, see, \eg\cite{dax2003}
...
confirmed in the next examples. There we will see that sometimes a
...
In essence, this example showed that the idea from the previous
section does indeed yield the desired behaviour. The method seems to
make more use of the information incorporated in the initial
eigenvector guess. This is precisely the purpose of the modification
of classic RQI as it was introduced in the previous section.
...
\begin{example}\label{ex:its}%
  ...
  while CRQI showed the same behaviour as describe above --
  independent of the matrix's size and hence the distance between the
  eigenvalues.
...
We conclude the discussion of the numerical examples with a summary of
the findings.
