...
\begin{proof}[Proof of Lemma~\ref{lem:eigs:atilde1}]
  ...
  the matrix.
  \begin{enumerate}
  \item Since only two eigenvalues of $\mat{E}$ are non-zero, the
    trace is the sum of these eigenvalues, \ie
    \begin{equation}
  ...
  \begin{align*}
    \mat{E}^2 &= (\vec{u} \vec{u}^\tp - \vec{v}_k \vec{v}_k^\tp)(\vec{u} \vec{u}^\tp - \vec{v}_k \vec{v}_k^\tp) \\
              ...
              &= \vec{u}\vec{u}^\tp + \vec{v}_k \vec{v}_k^\tp - \langle \vec{u}, \vec{v}_k \rangle \left(\vec{v}_k\vec{u}^\tp + \vec{u}\vec{v}_k^\tp \right)\,.
  \end{align*}
  Therefore,
  \begin{align*}
    \tr(\mat{E}^2) &= \norm*{\vec{u}}^2 + \norm*{\vec{v}_k}^2 - \langle \vec{u}, \vec{v}_k \rangle
...
\begin{proof}[Proof of Proposition~\ref{prop:atilde:eigenvalues}]
  ...
  $\tilde{\mat{A}}_{(0)}$ and use a classic result from perturbation
  theory\footnote{This result can be found, for example,
    in~\cite[66--69]{wilkinson} or~\cite[323]{golub1996}.} which
