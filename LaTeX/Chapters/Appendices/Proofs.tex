...
\begin{proof}[Proof of Lemma~\ref{lem:eigs:atilde1}]
  Since
  \begin{equation*}
    \norm{\tilde{\mat{A}}_{(1)}} = \norm{\gamma i \left( \vec{u} \vec{u}^\tp - \vec{v}_k \vec{v}_k^\tp \right)} = \gamma \norm{\smash{\underbrace{
  ...
  To compute the eigenvalues of $\mat{A}$ we use the following two
  ...
  the matrix.
  \begin{enumerate}
  \item The trace is the sum of the eigenvalues of $\mat{E}$, \ie
    \begin{equation}
  ...
  entries and thus, using the additivity of the trace, we get
  \begin{equation*}
    \tr(\mat{E}) = \norm*{\vec{u}} - \norm*{\vec{v}_k} = 0
  \end{equation*}
...
\begin{proof}[Proof of Proposition~\ref{prop:atilde:eigenvalues}]
  ...
  obtain
  \begin{align*}
    \vec{w}_j^\herm \tilde{\mat{A}}_{(0)} \vec{w}_j &= \gamma i \vec{w}_j^\herm  \left( \vec{u}\vec{u}^\herm - \vec{v}_k \vec{v}_k^\herm  \right) \vec{w}_j \\
                                                    &= \gamma i \left( \vec{w}_j^\herm\vec{u}\vec{u}^\herm\vec{w}_j - \vec{w}_j^\herm\vec{v}_k\vec{v}_k^\herm\vec{w}_j\right) \\
                                                    &= \gamma i \left({\langle \vec{w}_j, \vec{u}\rangle}^2 - {\langle \vec{w}_j, \vec{v}_k\rangle}^2 \right) \\
                                                    &=
                                                      \begin{cases}
                                                        \gamma i {\langle \vec{v}_j, \vec{u}\rangle}^2 & \text{ if } j \neq k\,, \\
                                                        \gamma i
                                                        \left(
                                                          {\langle
                                                            \vec{v}_j,
  ...
  \begin{align*}
    ...
    &=
      \lambda_j(\mat{A}) - \gamma i + \gamma i {\langle \vec{v}_j, \vec{u}\rangle}^2 \\
    &=
  ...
  \begin{align*}
    ...
    \vec{w}_j^\herm \tilde{\mat{A}}_{(0)} \vec{w}_j
    &=
      \lambda_k(\mat{A}) + \gamma i \left( {\langle \vec{v}_k, \vec{u}\rangle}^2 - 1 \right)
  \end{align*}
  for $j = k$ and thus
  \begin{equation*}
    \lambda_j(\tilde{\mat{A}}) =
