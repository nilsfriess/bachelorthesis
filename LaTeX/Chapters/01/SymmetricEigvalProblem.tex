...
Eigenvalues and eigenvectors of matrices play an important role in
many branches of science and engineering. A typical example from
physics is the computation of the natural frequencies of a structure,
\ie frequencies at which the structure is prone to vibrate. To
determine these frequencies, an eigenvalue problem needs to be solved.
Often, the actual problem is given for a differential operator and the
matrix eigenvalue problem arises when the operator is discretized.  To
accurately approximate the operator, the matrix often needs to be very
large. We usually assume that a good approximation of the desired
eigenvector is available. In problems from structural mechanics this
might be the case when one of the natural frequencies needs to be
recomputed after some of the model parameters (\eg certain material
properties) change.

In many cases the matrix for which the eigenvalues are sought is real
and symmetric. The task of finding eigenvalues and eigenvectors is
then referred to as the \emph{symmetric eigenvalue problem}. For
completeness, we collect some general facts from linear algebra on
eigenvalues and eigenvectors for the general complex case.

\begin{definition}
  Let $\mat{A} \in \C^{n \times n}$. A scalar $\lambda \in \C$ is
  called an \emph{eigenvalue} of $\mat{A}$ if there exists a nonzero
...
the identity matrix. This equation has a solution if and only if the
matrix $\mat{A} - \lambda \mat{I}$ is singular since by the definition
above $\vec{v}$ must not be the zero vector. Thus, eigenvalues of
...
stable~\cite[37]{golub2000eigenvalue}. Second, it is well-known that
even small perturbations in the coefficients of $\chi_{\mat{A}}(t)$
can lead to large errors in the calculation of the
roots~\cite[190]{trefethen1997}. Thus, other methods are necessary to
solve~\eqref{eq:eigvalproblem}, which has given rise to iterative
...
linear algebra preparing us for a discussion of such iterative methods
...
\begin{proposition}%
  ...
  \item There exists an orthonormal basis of $\C^n$ consisting of
    ...
    matrix, then the eigenvectors form an orthonormal basis of $\R^n$.
  ...
  \item If $\mat{A}$ is non-singular, then the eigenvalues of
    $\mat{A}^{-1}$ are given by
    $\lambda_1^{-1}, \dotsc, \lambda_n^{-1}$ with eigenvectors
    $\vec{v}_1, \dotsc, \vec{v}_n$.
