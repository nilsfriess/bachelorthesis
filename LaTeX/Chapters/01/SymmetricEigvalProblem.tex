...
matrix eigenvalue problem arises when the operator is discretised.  To
accurately approximate the operator, the matrix needs to be large but
when discretised with, for example, a finite difference or finite
element approach it is usually sparse. In a number of applications the
resulting matrix is real and symmetric and the task of finding
...
facts from linear algebra on eigenvalues and eigenvectors for the
...
\begin{definition}
  ...
  referred to as the \emph{spectrum}. To indicate that eigenvalues
  belong to a particular matrix $\mat{M}$ we sometimes write
  $\lambda(\mat{M})$.
\end{definition}
...
matrix $\mat{A} - \lambda \mat{I}$ is singular since by definition
$\vec{v}$ must not be the zero vector. Thus, eigenvalues of $\mat{A}$
are exactly the roots of the \emph{characteristic polynomial}
\[
...
\begin{remark}[Generalisations of eigenvalue problems]
  ...
  this thesis, we exclusively consider problems of the
